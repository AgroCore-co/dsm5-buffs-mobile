...
function shapeBufaloUpdate(p: any) {
  return clean({
    nome: p.nome,
    brinco: p.brinco,
    microchip: p.microchip,
...
function shapePesagemUpdate(p: any) {
  return clean({
    peso: p.peso,
    condicaoCorporal: p.condicaoCorporal ?? p.condicao_corporal,
    corPelagem: p.corPelagem ?? p.cor_pelagem,
...
function shapeSanitarioUpdate(p: any) {
  return clean({
    idMedicao: p.idMedicao ?? p.id_medicao,
    dtAplicacao: p.dtAplicacao ?? p.dt_aplicacao,
    dosagem: p.dosagem,
    unidade_medida: p.unidade_medida ?? p.unidadeMedida,
    doenca: p.doenca,
    necessita_retorno: p.necessita_retorno ?? p.necessitaRetorno,
    dtRetorno: p.dtRetorno ?? p.dt_retorno,
  });
}

function shapeReproducaoCreate(p: any) {
  // Campos por tipo de inseminação (contrato definido pela buffs-api):
  //   IA / IATF  → idSemen obrigatório; idBufalo PROIBIDO (servidor rejeita)
  //   TE         → idSemen (embrião); idBufalo opcional
  //   Monta Natural → idBufalo obrigatório; idSemen não se aplica
  //
  // idDoadora REMOVIDO: o backend deriva internamente de MaterialGenetico.idBufaloOrigem.
  const tipo = p.tipoInseminacao as string | undefined;
  const isIAorIATF = tipo === 'IA' || tipo === 'IATF';

  return cleanStrict({
    id: p.id,
    idPropriedade: p.idPropriedade,
    idSemen:  p.idSemen  ?? undefined,
    // idBufalo só é enviado para MN e TE — proibido em IA/IATF mesmo com valor
    idBufalo: !isIAorIATF ? (p.idBufalo ?? undefined) : undefined,
    idBufala: p.idBufala,
...
function shapeCicloUpdate(p: any) {
  // status NÃO entra: o backend deriva 'Seca'/'Em Lactação' de dtSecagemReal (computeStatus).
  return clean({
    idBufala: p.idBufala,
    idPropriedade: p.idPropriedade,
    dtParto: p.dtParto,
...
function shapeLoteUpdate(p: any) {
  return clean({
    nomeLote: p.nomeLote,
    idGrupo: p.idGrupo,
    tipoLote: p.tipoLote,
...
const RESOLVERS: Record<string, Resolver> = {
  bufalos: (op, p) => {
    ...
    if (op === 'UPDATE') return { endpoint: `/bufalos/${p.id}`, method: 'PATCH', body: shapeBufaloUpdate(p) };
  ...
  pesagens: (op, p) => {
    ...
    if (op === 'UPDATE') return { endpoint: `/dados-zootecnicos/${p.id}`, method: 'PATCH', body: shapePesagemUpdate(p) };
  ...
  eventos_sanitarios: (op, p) => {
    ...
    if (op === 'UPDATE') return { endpoint: `/dados-sanitarios/${p.id}`, method: 'PATCH', body: shapeSanitarioUpdate(p) };
  ...
  ciclos_lactacao: (op, p) => {
    ...
    if (op === 'UPDATE') return { endpoint: `/lactacao/${p.id}`, method: 'PATCH', body: shapeCicloUpdate(p) };
  ...
  lotes: (op, p) => {
    ...
    if (op === 'UPDATE') return { endpoint: `/lotes/${p.id}`, method: 'PATCH', body: shapeLoteUpdate(p) };
